...
void Ship::Load()
{
	...
	// Makes sure ship collides with enemies and enemy bullets
...
void Ship::Update(float deltaTime)
{
	...
	mFireTimer = std::max(0.0f, mFireTimer - deltaTime); // Fire timer

	// Dash handeling
	mDashTime = std::max(0.0f, mDashTime - deltaTime); // std::max to avoid negative time, counts how long dash lasts
	float dashSpeed = speed * (mDashTime > 0.0f ? mDashSpeedMul : 1.0f); // If dashing, multiply speed

	if (X::IsKeyPressed(X::Keys::E) && mDash > 0 && mDashTime <= 0.0f) // Start dash only if dash available and not already dashing
	{
		mDashTime = 0.5f;
		--mDash;
	}
	...
	{
		mPosition += X::Math::Vector2::Forward(mRotation) * dashSpeed * deltaTime; // Use dashSpeed which may be multiplied
	}
	...
	//{
	//	mWeapon = WeaponType::TripleShot;
	//}
	//else
	//{
	//	mWeapon = WeaponType::SingleShot;
	//}
	...
	if (X::IsKeyPressed(X::Keys::SPACE) && mFireTimer <= 0.0f && mBulletPool)
	{
		...
		else
		{
			...
			for(int i = -1; i <= 1; ++i)
			{
				float r = mRotation + i * spread; // Calculate rotation for each bullet
...
{
	mShieldTime = std::max(mShieldTime, seconds); // Extend shield time if new shield is longer
}

void Ship::DoNovaShot()
{
	if (!mBulletPool) // Safety check
	{
		return;
	}

	const int bulletsPerRing = 36; // 10 degree increments
	const float life = 2.0f;
	const float muzzleDist = 50.0f;

	for (int ring = 0; ring < 2; ++ring) // Two rings of bullets
	{
